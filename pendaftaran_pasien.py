...
keys_to_init = ['nama_lengkap', 'nik', 'no_hp', 'blok_mes', 'tgl_lahir', 'lokasi_kerja', 'logged_in', 'proses_simpan']
for key in keys_to_init:
    if key not in st.session_state:
        st.session_state[key] = "" if key != 'logged_in' and key != 'proses_simpan' else False
...
if is_pasien_mode:
    ...
    st.sidebar.success(f"🔓 {st.session_state.get('role')}: {st.session_state.get('username')}")
    menu_list = ["Pendaftaran Pasien", "Rekam Medis / 病历", "SKD / 医生证明", "Dashboard Analitik"]
