...
def buat_formulir_otomatis(data, petugas):
    # 1. Fungsi Clean di Paling Atas agar bisa diakses Kop & Data Pasien
    def clean(text):
        if not text: return "-"
    ...
    # PERBAIKAN LOGO: HJF dirapatkan (x=24) ke Harita (x=12) sesuai gambar referensi
    if path_harita: pdf.image(path_harita, x=12, y=13, h=10)
    if path_hjf:    pdf.image(path_hjf, x=24, y=13, h=10) # Rapatkan ke kiri
    
    # PERBAIKAN TEKS KOP: Seluruh teks kop (Kuning) digeser ke kiri (x=36) sdikit dari logo HJF
    ...
    # Logo SMK3 di pojok kanan area alamat (diberi jarak spasi dari teks)
    ...
    # KHFO-000000 (Teks di gambar manual)
    pdf.set_font("helvetica", "B", 36) # Font diperbesar raksasa
    no_rm = data.get('no_rm', 'KHFO-000000') 
    pdf.cell(190, 15, clean(no_rm), ln=True, align="R")

    # PERBAIKAN A4: Menghapus area karakter aneh (area panah merah) - Kotak RM sekarang BERSIH
    ...
    # --- PERBAIKAN BAWAH A4: PERPANJANG KOTAK SURAT PERNYATAAN & TTD AGAR LEGA (SISI MERAH) ---
    pdf.ln(5)
    y_p_box = pdf.get_y()
    
    # Kotak bawah diperpanjang (tinggi diperbesar dari 55mm ke 65mm) agar terlihat sempurna di kertas A4
    # Koordinat x,y,w,h (10, y_p_box, 190, 65)
    pdf.rect(10, y_p_box, 190, 65) 

    # Area Teks Pernyataan (X/Y di dalam kotak)
    pdf.set_xy(12, y_p_box + 2)
    pdf.set_font("helvetica", "B", 11)
    ...
    pdf.set_x(12)
    pdf.multi_cell(186, 5, pernyataan) # Multi_cell untuk teks panjang

    # Area Tanda Tangan (di pojok kanan kotak diperpanjang)
    pdf.ln(10) # Jarak diperlebar ke bawah
    ...
    pdf.ln(25) # Jarak diperlebar ke bawah agar tidak dempet saat diprint
